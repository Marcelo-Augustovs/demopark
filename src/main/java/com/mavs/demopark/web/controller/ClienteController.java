...
public class ClienteController {
    ...
    @Operation(summary = "Recuperar lista de clientes",
        description = "Requisição exige uso de bearer token. Acesso restrito a Role='ADMIN' ",
        security = @SecurityRequirement(name = "security"),
        parameters = {
    ...
    @GetMapping
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<PageableDto> getAllCliente(@Parameter(hidden = true) @PageableDefault(size = 5,sort = {"nome"}) Pageable pageable){
        Page<ClienteProjection> clientes = clienteService.getAllClientes(pageable);
